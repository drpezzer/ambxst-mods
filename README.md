...
| [multi-monitor-fixes](packages/multi-monitor-fixes) | Dual Screen Single Bar (the bar follows the focused screen, windows moving in step), staged chrome exit on a screen a fullscreen window covers, auto-hide bars on the same slide, monitor hotplug without a reload, wallpaper-manager handoff, notch and fullscreen-detection fixes |
