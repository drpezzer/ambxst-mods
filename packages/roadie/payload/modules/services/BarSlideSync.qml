...
// Ambxst Roadie (drpezzer.roadie): keeps the compositor's window move in step with the
...
//
// This is the same idea ShellTransitions applies to the shell enter/leave,
// on its own curve and state file. The restore is guarded: it
...
Singleton {
    ...
    readonly property string stateFile: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/ambxst/roadie-barslide-windowsmove.json"
